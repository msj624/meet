...
jQuery(document).ready(function($) {
  ...
  $('.at-upload-button, , .at-change_image_button').click( function() {
  ...
  */
    ...
    jQuery(".mupload_img_holder").each(function(i,v){
      if (jQuery(this).next().next().val() != ''){
        if (!jQuery(this).children().size() > 0){
          jQuery(this).append('<img src="' + jQuery(this).next().next().val() + '" style="width: auto;" />');
          jQuery(this).next().next().next().val("Change Image");
          jQuery(this).next().next().next().removeClass('at-upload_image_button').addClass('at-change_image_button');
        } else {
            jQuery(this).find('img').attr('src',jQuery(this).next().next().val());
        }
      }
    });
  }
  ...
  //upload button
    var formfield1;
    var formfield2;
    jQuery('.at-upload_image_button, .at-change_image_button').live('click',function(e){
