...
function qafphome_shortcode( $atts ) {
	...
	if ( $id ) { // Show a single entry //
		...
		while( $qafp_faqs->have_posts() ): $qafp_faqs->the_post();
			...
			global $post;
			$qafp_shortcode_output .= '<div id="qafp-faq' . $i . '" class="qafp-faq">
			<div class="qafp-faq-title" style="' . $qafp_options['titlecss'] . '"><a class="qafp-faq-anchor" href="' . get_permalink() . '">' . get_the_title() . '</a></div>
			<div class="qafp-faq-answer">' . apply_filters( 'the_content', get_the_content() );

			if ( $permalinks == true && $ratings == true ) $qafp_shortcode_output .= '<p class="qafp-faq-meta qafp-post-like">' . getPostLikeLink($post->ID) . '<a class="qafp-permalink" href="' . get_permalink() . '" style="margin-left:1em;">' . __( 'Permalink', 'qa-focus-plus') . '</a></p>
			';					

			if ( $permalinks != true && $ratings == true ) $qafp_shortcode_output .= '<p class="qafp-faq-meta qafp-post-like">' . getPostLikeLink($post->ID) . '</p>
			';

			if ( $permalinks == true && $ratings != true ) $qafp_shortcode_output .= '<p class="qafp-faq-meta qafp-post-like"><a class="qafp-permalink" href="' . get_permalink() . '">' . __( 'Permalink', 'qa-focus-plus') . '</a></p>
			';

			if ( $hr == true ) $qafp_shortcode_output .= '<hr />
			';
		...
		$home_link = null;
		if ( is_page($qafp_options['faq_slug']) ) {
			$page_title = get_the_title( get_page_by_path($qafp_options['faq_slug']) );
			$home_link = '<p>
			<a href="' . home_url() . '/' . $qafp_options['faq_slug'] . '" title="' . $page_title . '">&larr; ' . $page_title . '</a>
			</p>
			';
		}
		...
		$qafp_shortcode_output .=  $home_link . '<div class="qafp-category">
		...
		$qafp_shortcode_output .= '</div>
		' . $home_link . '</div>'; // .qafp-category .qafp-faqs
		...
		if ( $categories ) {
			...
			foreach ( $categories as $category ) {
				...
				while( $qafp_faqs->have_posts() ): $qafp_faqs->the_post();
					...
					$qafp_shortcode_output .= '<div id="qafp-faq' . $i . '" class="qafp-faq cf">
					<div class="qafp-faq-title" style="' . $qafp_options['titlecss'] . '"><a class="qafp-faq-anchor" href="' . get_permalink() . '">' . get_the_title() . '</a></div>
					';
					
					if ( $excerpts == true ) {
						$qafp_shortcode_output .= '<div class="qafp-faq-answer">' . apply_filters( 'the_content', get_the_excerpt() );
					} else { 
						$qafp_shortcode_output .= '<div class="qafp-faq-answer">' . apply_filters( 'the_content', get_the_content() );
					}
				...
				if ( $catlink == true ) {
					...
					$qafp_shortcode_output .= '<a class="qafp-show-more" href="' . $url . '">' . __( 'View category', 'qa-focus-plus' ) . ' &rarr;</a>
					';	
				}
			...
			<p class="qafp-faq-meta qafp-post-like">' . getPostLikeLink($post->ID) . '<a class="qafp-permalink" href="' . get_permalink() . '" style="margin-left:1em;">' . __( 'Permalink', 'qa-focus-plus') . '</a></p>
				';					
